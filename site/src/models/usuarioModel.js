...
function entrar(username, senha) {
    ...
    var instrucao = `
        SELECT * FROM Usuario WHERE username = '${username}' AND senha = md5('${senha}');
    `;
...
function cadastrar(username, nome, email, senha, contato) {
    ...
    var instrucao = `
        INSERT INTO Usuario (username, senha, nome, email, telefone) VALUES ('${username}', md5('${senha}'), '${nome}', '${email}', '${contato}');
    `;
    console.log("Executando a instrução SQL: \n" + instrucao);
    return database.executar(instrucao);
}
